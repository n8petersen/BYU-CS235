...
private:
	...
	int hash(const K& key)
	{
		...
		int k = 1;
		int newIndex = hashIndex % capacity;

		while (1)
		{
			// If the key slot is empty, insert it there
			if (myPairs[newIndex].first.length() == 0) 
			{
				cout << " " << "Insert " << key << endl;
				myPairs[newIndex].first = key;
				break;
			}

			// If the key already exists, don't insert again
			else if (myPairs[newIndex].first == key) 
			{
				break;
			}

			// Otherwise, where there is a collision
			cout << endl << " " << key << " collided with " << myPairs[newIndex].first; 
			
			// Add k to Index,
			// Mod the index,
			// Increment k by 2
			hashIndex = (hashIndex + k);	
			newIndex = hashIndex % capacity; 
			k += 2;	
		}
		return newIndex;
	}
