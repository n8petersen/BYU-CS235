...
// 
// NOTE 2:	This is a recycled class from the BST lab. There's probably  a lot of stuff here
//			that isn't applicable to the pokemon lab.
...
private:
	...
	bool infixOrder(Node* node, stringstream& out, string& comma) const;


public:
	BST() : root_(NULL), tree_size(0) {}
	~BST() { clearTree(); }


	/* Deep Copy Constructor */
	BST(const BST& bst) : root_(NULL), tree_size(0) 
	{
	...
	BST& operator=(const BST& rhs)
	{
		...
		this->tree_size = the_copy.tree_size;
		return *this;
	}
...
template<typename T>
bool BST<T>::infixOrder(Node* node, stringstream& out, string& comma) const
{
	if (node == NULL)
	{
		return false;
	}
	infixOrder(node->left_, out, comma);
	out << comma << node->data_;
	comma = ",";

	infixOrder(node->right_, out, comma);
	return true;
}
